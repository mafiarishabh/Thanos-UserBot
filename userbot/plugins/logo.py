...
async def lg1(REBEL):
    event = await edit_or_reply(REBEL, "`Processing.....`")
    fnt = await get_font_file(REBEL.client, "@VegaFonts")
    if REBEL.reply_to_msg_id:
        rply = await REBEL.get_reply_message()
    ...
    text = REBEL.pattern_match.group(1)
    if len(text) <= 8:
        font_size_ = 150
        strik = 10
    elif len(text) >= 9:
