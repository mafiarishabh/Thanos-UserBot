...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query == "@thanosBot_chat":
            rev_text = query[::-1]
            ...
            result = builder.article(
                ...
                buttons=[
                    ...
                    [
                        custom.Button.url(
                            "🔰 REPO 🔰", "https://github.com/SURTURBOT/thanos-UserBo"),
                        custom.Button.url
                    (
                            "🔰 TUTORIAL 🔰", ""
                    )
    ...
    async def page(event):
        if not event.query.user_id == bot.uid:
            return await event.answer(
                "HELLO THERE. PLEASE MAKE YOUR OWN ThanosBot AND USE. © ThanosBot ™",
                cache_time=0,
                alert=True,
            )
