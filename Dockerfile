...
RUN wget -O opencv.zip https://github.com/opencv/opencv/archive/Thanosbot.zip && unzip opencv.zip && mv -f opencv-thanosbot /usr/bin/ && rm opencv.zip
RUN git clone https://github.com/mafiarishabh/Thanos-UserBot /root/userbot
RUN mkdir /root/userbot/bin/
WORKDIR /root/userbot/
RUN chmod +x /usr/local/bin/*
RUN pip3 install -r requirements.txt
CMD ["bash","./mafiarishabh/start.sh"]
